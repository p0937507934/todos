...
function App() {
  ...
  const handleCreate = async () => {
    const res = await axios.post("http://127.0.0.1:3001/todos", { data: data });
    alert("新增成功!");
  };
  ...
  const handleUpdate = async(id) => {
    const res = await axios.put("http://127.0.0.1:3001/todos",{id:id,data:newData})
    alert("修改成功!")
  };
  const handleDelete = async(id) => {
    const res = await axios.delete("http://127.0.0.1:3001/todos",{ data: { id:id }})
    alert("刪除成功!")
  };
